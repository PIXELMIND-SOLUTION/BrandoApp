...
class SharingOption {
  ...
  factory SharingOption.fromJson(Map<String, dynamic> json) {
    return SharingOption(
      shareType: json['shareType'] ?? '',
      type: json['type'] ?? '',
      // API sends flat "monthlyPrice" and "dailyPrice"
      monthlyPrice: (json['monthlyPrice'] as num?)?.toInt() ?? 0,
      dailyPrice: (json['dailyPrice'] as num?)?.toInt() ?? 0,
    );
  }


  int get nonAcMonthlyPrice => monthlyPrice;
  int get nonAcDailyPrice => dailyPrice;
  int get acMonthlyPrice => monthlyPrice;
  int get acDailyPrice => dailyPrice;
}
